...
int main(int argc,char ** argv)
{
    ...
    cpu_t * cpu       = init_cpu(CPUID,offset,memory);
    loader(memory,bin_file,offset);
    pthread_create(&runcpu,       NULL,cpu_main_loop,cpu);
    pthread_mutex_t lock;
    pthread_mutex_init(&lock,NULL);
#if 1
    window_t * window = init_screen(cpu,&lock);
